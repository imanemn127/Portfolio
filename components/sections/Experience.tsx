...
export function Experience() {
  ...
  return (
    <section ref={sectionRef} id="experience" className="section-padding px-6 lg:px-16" aria-label="Experience">
                      ...
                      <div className="flex-1">
                        <div className="mb-5">
                          ...
                          {"projectTitle" in exp && exp.projectTitle && (
                            <p className="font-mono text-[11px] text-[#5a5c70] leading-relaxed max-w-lg mt-1 italic">
                              {exp.projectTitle as string}
                            </p>
                          )}
                        </div>
                        ...
                        {/* Repo links — Inria only */}
                        {"repos" in exp && Array.isArray(exp.repos) && (
                          <div className="flex flex-wrap gap-2 mb-5">
                            {(exp.repos as { label: string; url: string }[]).map((repo) => (
                              <a
                                key={repo.url}
                                href={repo.url}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded font-mono text-[11px] border transition-all duration-200"
                                style={{
                                  borderColor: `${exp.color}30`,
                                  color: exp.color,
                                ...
                                onMouseEnter={e => {
                                  ...
                                  el.style.backgroundColor = `${exp.color}18`
                                  el.style.borderColor = `${exp.color}55`
                                }}
                                onMouseLeave={e => {
                                  const el = e.currentTarget
                                  el.style.backgroundColor = `${exp.color}08`
